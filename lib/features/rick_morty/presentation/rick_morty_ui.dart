...
class RickMortyUI extends UI<RickMortyViewModel> {
  ...
  Widget build(BuildContext context, RickMortyViewModel viewModel) {
    ...
    return Scaffold(
        ...
        body: _CharacterListWidget(
            isLoading: false,
            child: RefreshIndicator(
                onRefresh: () async {},
                child: ListView.builder(
                    itemCount:
                        response['data']?['characters']?['results']?.length,
                    itemBuilder: (context, index) {
                      return CharacterCardWidget(
                          characterImage: 'assets/rickandmorty.jpeg',
                          characterName: results[index]['name']!,
                          characterSpecies: results[index]['species']!,
                          characterGender: results[index]['gender']!,
                          characterStatus: results[index]['status']!);
                    }))));
  }
}
